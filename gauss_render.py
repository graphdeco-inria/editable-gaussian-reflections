# Adapted from https://github.com/hbb1/torch-splatting/tree/main
...
def project_to_imageplane(points3D, camera):
    return projection_ndc(points3D, viewmatrix=camera.world_view_transform, projmatrix=camera.projection_matrix)[0][:, :2]

def compute_gaussian_alphas(dx, camera, pc, hits):
    means3D = pc.get_xyz
    ...
    # g_xy = project_to_imageplane(means3D, camera)[hits.gaussian_ids]
    # "2D viewport coordinates of the gaussian centroids for each ray/gaussian pair"
    # r_xy = project_to_imageplane(ray_imageplane_hitpoints_ws.flatten(0, 1), camera)[hits.ray_ids]
    # "2D viewport coordinates each ray's intersection with the image plane for each ray/gaussian pair"
    
    sorted_cov2d = cov2d[hits.gaussian_ids] 
    sorted_conic = sorted_cov2d.inverse() 
    sorted_opacity = opacity[hits.gaussian_ids]
    ...
    return gauss_weight #!!1
    
    # alpha = gauss_weight * sorted_opacity.squeeze(-1) #!!!
    return alpha
