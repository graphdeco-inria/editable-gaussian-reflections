...
def cook_torrence(normals, view_dir, light_dir, base_color, metalness, roughness):
    ...
    specular = (
        D * G / (4.0 * dotpos(normals, view_dir) * dotpos(normals, light_dir) + 1e-8)
    )
...
def D_ggx(normals, halfway, roughness):
    alpha = roughness**2
    return (
        alpha**2 / (math.pi * (dotpos(normals, halfway) ** 2 * (alpha**2 - 1) + 1)) ** 2
    )


def G_ggx(normals, view_dir, light_dir, roughness):
    k_direct = (roughness**2 + 1) ** 2 / 8
    G_v = dotpos(normals, view_dir) / (
        dotpos(normals, view_dir) * (1.0 - k_direct) + k_direct
    )
    G_l = dotpos(normals, light_dir) / (
        dotpos(normals, light_dir) * (1.0 - k_direct) + k_direct
    )
    return G_v * G_l
...
def project(ws_pts, view_idx, poses):
    ...
    cs_pts = (centered_pts.unsqueeze(-2) @ c2w[:, :3, :3]).squeeze(
        -2
    )  # ?? squeeze on 2 or 1
