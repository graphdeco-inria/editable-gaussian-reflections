...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    pybind11::class_<Config>(m, "Config")
        .def(pybind11::init<int, const std::string&, bool>(),
             pybind11::arg("version") = 1, pybind11::arg("name") = "default", pybind11::arg("debug") = false)
        ...
        .def("__repr__", [](const Config &c) {
            return "<Config version=" + std::to_string(c.version) +
                   " name='" + c.name + "' debug=" + (c.debug ? "True" : "False") + ">";
        });
}