...
__device__ float sigmoid_act(float x) {
...
__device__ float3 backward_sigmoid_act(float3 dL_dy, float3 y) {
    return { backward_sigmoid_act(dL_dy.x, y.x), backward_sigmoid_act(dL_dy.y, y.y), backward_sigmoid_act(dL_dy.z, y.z) };
}

__device__ float softplus_act(float x) {
...
__device__ float abs_act(float x) {
    return abs(x);
}
...
__device__ float3 backward_softplus_act(float3 dL_dy, float3 x, float3 y) {
    return { backward_softplus_act(dL_dy.x, x.x, y.x), backward_softplus_act(dL_dy.y, x.y, y.y), backward_softplus_act(dL_dy.z, x.z, y.z) };
}

// RELU

__device__ float relu_act(float x) {
    return max(0.0f, x);
}
...
__device__ float3 backward_relu_act(float3 dL_dy, float3 y) {
    return { backward_relu_act(dL_dy.x, y.x), backward_relu_act(dL_dy.y, y.y), backward_relu_act(dL_dy.z, y.z) };
}

// CLIPPED RELU 

__device__ float clipped_relu_act(float x) {
...
__device__ float3 backward_clipped_relu_act(float3 dL_dy, float3 y) {
    return { backward_clipped_relu_act(dL_dy.x, y.x), backward_clipped_relu_act(dL_dy.y, y.y), backward_clipped_relu_act(dL_dy.z, y.z) };
}

// EXP

__device__ float exp_act(float x) {
    return exp(x);
}
...
__device__ float backward_exp_act(float dL_dy, float y) {
    return dL_dy * y;
}
__device__ float3 backward_exp_act(float3 dL_dy, float3 y) {
    return { backward_exp_act(dL_dy.x, y.x), backward_exp_act(dL_dy.y, y.y), backward_exp_act(dL_dy.z, y.z) };
}
...
// }
        ...
        #define READ_OPACITY(gaussian_id) softplus_act(params.gaussian_opacity[gaussian_id])
    #else
        #define READ_OPACITY(gaussian_id) sigmoid_act(params.gaussian_opacity[gaussian_id])
    #endif
    #define READ_ROTATION(gaussian_id) normalize_act(params.gaussian_rotations[gaussian_id])
    #if USE_LEVEL_OF_DETAIL == true && ADD_LOD_MEAN_TO_SCALE == true
        #define READ_SCALE(gaussian_id) (exp_act(params.gaussian_scales[gaussian_id]) + params.gaussian_lod_mean[gaussian_id])
    #else
    ...
    #define READ_LOD_SCALE(gaussian_id) exp_act(params.gaussian_lod_scale[gaussian_id])
    
    #define READ_ROUGHNESS(gaussian_id) clipped_relu_act(params.gaussian_roughness[gaussian_id])
