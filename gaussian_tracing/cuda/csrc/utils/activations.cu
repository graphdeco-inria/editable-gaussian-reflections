...
__device__ float backward_sigmoid_act(float dL_dy, float y) { return dL_dy * y * (1.0f - y); }
...
__device__ float backward_softplus_act(float dL_dy, float x, float y) { return dL_dy / (1.0f + exp(-x)); }
...
__device__ float backward_relu_act(float dL_dy, float y) { return dL_dy * (y >= 0.0f ? 1.0f : 0.0f); }
...
__device__ float3 backward_identity_act(float3 dL_dy, float3 y) { return {dL_dy.x, dL_dy.y, dL_dy.z}; }
