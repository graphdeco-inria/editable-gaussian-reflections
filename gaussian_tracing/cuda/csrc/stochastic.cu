    ...
    for (int ki = 0; ki < TILE_SIZE; ki++) {
        for (int kj = 0; kj < TILE_SIZE; kj++) {
            ...
            float3 world_direction = normalize(
                ...
                params.camera_rotation_w2c[1] * y -
                params.camera_rotation_w2c[2]);

            float3 local_origin =
                world_origin -
                READ_MEAN(gaussian_id); // todo pick out from matrix & verify ok
            float3 local_direction = make_float3(
                ...
                dot(make_float3(
                        ...
                        world_to_local[1].z,
                        world_to_local[2].z),
                    world_direction)); // todo verify that it matches the value
                                       // obtained for the tile direction
            ...
            // if (hit) {
            // }
            output_rgb[ki * TILE_SIZE + kj] +=
                READ_RGB(gaussian_id) / BUFFER_SIZE;
...
if (false) {
    ...
    register float weight_buffer[BUFF_SIZE];
    fill_array(weight_buffer, BUFF_SIZE, 1.0f);
    while (hit_idx != 999999999) {
        float distance = params.all_distances[hit_idx];
        float alpha = params.all_alphas[hit_idx];
        auto gaussian_id = params.all_gaussian_ids[hit_idx];
    ...
    for (int i = 0; i < BUFF_SIZE; i++) {
        if (gaussian_id_buffer[i] != NULL_GAUSSIAN_ID) {
            output_rgb[0] += READ_RGB(gaussian_id_buffer[i]) /
                             (1.0 - powf(1.0 - weight_buffer[i], BUFF_SIZE)) *
                             weight_buffer[i];
...
if (true) {
    ...
    hit_idx = params.prev_hit_per_pixel[ray_id];
    register float T_buffer[BUFF_SIZE];
    fill_array(T_buffer, BUFF_SIZE, 1.0f);
    while (hit_idx != 999999999) {
        float distance = params.all_distances[hit_idx];
        float alpha = params.all_alphas[hit_idx];
        uint32_t prev_hit = params.all_prev_hits[hit_idx];
    ...
    // Composite all gaussians, estimating the transmittance with that of the
    // nearest gaussian present in the buffer
    hit_idx = params.prev_hit_per_pixel[ray_id];
    ...
    while (hit_idx != 999999999) {
        ...
        for (int i = 0; i < BUFF_SIZE - 1; i++) {
            if (distances_buffer[i + 1] >
                distance) { // todo error for gaussians which are exactly
