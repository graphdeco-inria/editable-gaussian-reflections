...
__device__ __host__ float compute_scaling_factor(float opacity, float alpha_threshold, float exp_power) {
        ...
        #if EXP_POWER == 1
            return max(MIN_SCALING_FACTOR, opacity <= alpha_threshold ? 0.0 : sqrtf(2.0f * log(opacity / alpha_threshold)));
        #else
            float k = 2.0f * exp_power;
            return max(MIN_SCALING_FACTOR, opacity <= alpha_threshold ? 0.0 : powf(k * log(opacity / alpha_threshold), 1.0f / k));
            // return max(MIN_SCALING_FACTOR, opacity <= alpha_threshold ? 0.0 : powf(2 * log(opacity / alpha_threshold), 1.0f / 2^k));
        #endif
    #else
...
// x = +- (k * log(y))^(1/k)
    ...
    __device__ float eval_gaussian(float3 local_hit, float exp_power) {
        float k = 2.0f * exp_power;
            ...
            float d = powf(fabs(local_hit.x), k) + powf(fabs(local_hit.y), k) + powf(fabs(local_hit.z), k);
            return exp(-d / k);
        #else 
...
__device__ float compute_alpha(float guassval, float opacity, float alpha_threshold) {
    ...
    __device__ float compute_alpha_windowing(float blur_level, float gaussian_lod_mean, float guassian_lod_scale) {
        return max(0.0f, 1.0 - powf(
