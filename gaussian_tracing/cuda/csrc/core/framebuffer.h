...
struct Framebuffer {
    ...
    __device__ void update_accumulators(Pixel &pixel) {
        ...
        for (int step = 0; step < MAX_BOUNCES + 1; step++) {
            ...
            accumulated_transmittance[pixel.id + num_pixels * step] +=
                pixel.output_transmittance[step];
            pixel.output_transmittance[step] =
                (accum_transmittance + pixel.output_transmittance[step]) / float(accum_count + 1);
            auto accum_total_transmittance =
                accumulated_total_transmittance[pixel.id + num_pixels * step];
            accumulated_total_transmittance[pixel.id + num_pixels * step] +=
                pixel.output_total_transmittance[step];
            pixel.output_total_transmittance[step] =
                (accum_total_transmittance + pixel.output_total_transmittance[step]) /
                float(accum_count + 1);
            ...
            pixel.output_depth[step] =
                (accum_depth + pixel.output_depth[step]) / float(accum_count + 1);
            ...
            pixel.output_normal[step] =
                (accum_normal + pixel.output_normal[step]) / float(accum_count + 1);
            ...
            pixel.output_roughness[step] =
                (accum_roughness + pixel.output_roughness[step]) / float(accum_count + 1);
    ...
    __device__ void write_outputs(const Pixel &pixel) {
        for (int step = 0; step < MAX_BOUNCES + 1; step++) {
            ...
            output_total_transmittance[pixel.id + num_pixels * step] =
                pixel.output_total_transmittance[step];
...
struct FramebufferDataHolder : torch::CustomClassHolder {
    ...
    FramebufferDataHolder(uint32_t image_width, uint32_t image_height) {
        ...
        output_roughness =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 1}, CUDA_FLOAT32);
        output_transmittance =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 1}, CUDA_FLOAT32);
        output_total_transmittance =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 1}, CUDA_FLOAT32);
        output_brdf = torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 3}, CUDA_FLOAT32);
        output_ray_origin =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 3}, CUDA_FLOAT32);
        output_ray_direction =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 3}, CUDA_FLOAT32);
        ...
        accumulated_transmittance =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 1}, CUDA_FLOAT32);
        accumulated_total_transmittance =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 1}, CUDA_FLOAT32);
        accumulated_depth =
            torch::zeros({MAX_BOUNCES + 1, image_height, image_width, 1}, CUDA_FLOAT32);
        accumulated_normal =
    ...
    Framebuffer reify() {
        return Framebuffer{
            ...
            .output_total_transmittance =
                reinterpret_cast<float *>(output_total_transmittance.data_ptr()),
            ...
            .accumulated_transmittance =
                reinterpret_cast<float *>(accumulated_transmittance.data_ptr()),
            .accumulated_total_transmittance =
                reinterpret_cast<float *>(accumulated_total_transmittance.data_ptr()),
            ...
            .accumulated_sample_count =
                reinterpret_cast<int *>(accumulated_sample_count.data_ptr()),
    ...
    static void bind(torch::Library &m) {
        m.class_<FramebufferDataHolder>("Framebuffer")
            ...
            .def_readonly(
                "output_total_transmittance", &FramebufferDataHolder::output_total_transmittance)
            ...
            .def_readonly(
                "accumulated_transmittance", &FramebufferDataHolder::accumulated_transmittance)
            .def_readonly(
                "accumulated_total_transmittance",
                &FramebufferDataHolder::accumulated_total_transmittance)
            ...
            .def_readonly(
                "accumulated_sample_count", &FramebufferDataHolder::accumulated_sample_count)
