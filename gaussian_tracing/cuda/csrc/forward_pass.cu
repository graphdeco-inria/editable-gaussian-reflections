...
__device__ void froward_pass(
    ...
    float endpoint = -1.0f;
            ...
            if (!*params.cheap_approx) {
                ...
                uint32_t ray_id_downsampled = (idx.y / CHEAP_TMAX_DOWNSAMPLING) * params.image_width + idx.x / CHEAP_TMAX_DOWNSAMPLING;
                far_plane = min(far_plane, params.t_maxes[ray_id_downsampled] + 1e-8);
            }
        #endif

        uint32_t full_T_uint[TILE_SIZE*TILE_SIZE]; fill_array(full_T_uint, TILE_SIZE*TILE_SIZE, __float_as_uint(1.0f));

        // * Traverse BVH
        for (uint32_t i = 0; i < NUM_SLABS; i++) {
            float slab_tmin = near_plane + (far_plane - near_plane) / NUM_SLABS * i;
            float slab_tmax = near_plane + (far_plane - near_plane) / NUM_SLABS * (i + 1);
            ...
            if (output_t[0].y > params.transmittance_threshold) { 
                ...
                uint32_t uint_lod_by_distance = __float_as_uint(lod_by_distance);
                    ...
                    // float min_normalized_lod = (*params.camera_znear * lod_by_distance) / *params.max_lod_size;
                    // // if (ray_id == 777) {
                    // //     printf("min_normalized_lod %f, camera_znear %f, lod_by_distance %f, max_lod_size %f\n", min_normalized_lod, *params.camera_znear, lod_by_distance, *params.max_lod_size);
                    // // }
                    // uint8_t mask = 0;
                    // for (int l = 0; l < 8; l++) {
                ...
                optixTraverse(
                    ...
                    0.0f,   // rayTime
                    #if USE_LEVEL_OF_DETAIL_MASKING == true
                        OptixVisibilityMask( mask ), // todo when slab rendering, select all levels of the hierarchy that the slab overlaps with 
                    #else 
                    ...
                    #if TILE_SIZE == 1
                        full_T_uint[0],
                        reflected_origin_x, reflected_origin_y, reflected_origin_z
                    #elif TILE_SIZE == 2
                        printf("dead code\n");
                        full_T_uint[0], full_T_uint[1], full_T_uint[2], full_T_uint[3]
                    #elif TILE_SIZE == 3
                        printf("dead code\n");
                        full_T_uint[0], full_T_uint[1], full_T_uint[2], full_T_uint[3], full_T_uint[4], full_T_uint[5], full_T_uint[6], full_T_uint[7], full_T_uint[8]
                    #else
                        // raise an error
    ...
    #if FUSED_MESH == true
        for (int k = 0; k < TILE_SIZE*TILE_SIZE; k++) {
            output_rgb[k][0] = params.output_rgb[ray_id]; // *** tmp stuff for debugging
        }
        return; 
    #endif
    ...
    #endif
    for (int iteration = 0; iteration < MAX_ITERATIONS && tmin < 99.9f; iteration++) { 
        #if STORAGE_MODE == PER_PIXEL_LINKED_LIST
            ...
            for (int ki = 0; ki < TILE_SIZE; ki++) for (int kj = 0; kj < TILE_SIZE; kj++) { 
                params.full_T[ray_id + ki * params.image_width + kj] = 1.0f; // todo this was a tmp solution, doesn't work with multibounce
            }
            ...
            optixTraverse(
                ...
                0,    // SBT stride 
                0,
                floats[0], floats[1], floats[2], floats[3], floats[4], floats[5], floats[6], floats[7], floats[8], floats[9], floats[10], floats[11], floats[12], floats[13], floats[14], floats[15],
                gaussian_ids[0], gaussian_ids[1], gaussian_ids[2], gaussian_ids[3], gaussian_ids[4], gaussian_ids[5], gaussian_ids[6], gaussian_ids[7], gaussian_ids[8], gaussian_ids[9], gaussian_ids[10], gaussian_ids[11], gaussian_ids[12], gaussian_ids[13], gaussian_ids[14], gaussian_ids[15]
            );
        ...
        // * Integrate the values
            ...
            float A[TILE_SIZE*TILE_SIZE]; fill_array(A, TILE_SIZE*TILE_SIZE, 0.0f);
            float D[TILE_SIZE*TILE_SIZE]; fill_array(D, TILE_SIZE*TILE_SIZE, 0.0f);
            float D2[TILE_SIZE*TILE_SIZE]; fill_array(D2, TILE_SIZE*TILE_SIZE, 0.0f);
        #endif
        ...
        for (int i = 0; i < BUFFER_SIZE; i++) {
            ...
            tmin = max(distance, tmin); // todo: fails if its not a max, but the values should already be sorted??
            ...
            if (distance < 99.9f) {  
                ...
                float3 gaussian_rgb = READ_RGB(gaussian_id);
                    ...
                    float3 gaussian_position = params.gaussian_position[gaussian_id];
                #endif
                    ...
                    float gaussian_metalness = params.gaussian_metalness[gaussian_id];
                #endif
                ...
                num_hits++; //! was incorrect for tiling, review
                    ...
                    if (*params.iteration == 0 && ray_id == DEBUG_DUMP_PIXEL_ID) {
                        dump_i = params.dump->idx++; // starts at -1 so the indexing works out 
                    }
                #endif
                    ...
                    #endif
                    float half_chord_length = params.all_half_chord_lengths[idxes[i]];
                    if (ray_id == 777) {
                        printf("endpoint: %f, distance: %f, half_chord_length: %f\n", endpoint, distance, half_chord_length);
                    }
                    // if ((endpoint > 0.0f) && (distance > endpoint) && (distance - endpoint) <= half_chord_length) {
                    //     if (current_cluster < NUM_CLUSTERS - 1) {
                    //         current_cluster++; // todo starting for the secondary ray, ignore the first cluster
                    //     }
                    // }
                ...
                int new_idx;
                for (int k = 0; k < TILE_SIZE*TILE_SIZE; k++) {
                    // #if TILE_SIZE > 1 // this is actually slower than just stopping at the same point for all subpixels
                    ...
                    // #endif
                        ...
                        if (BBOX_PADDING > 0.0f) {
                            ...
                            local_origin = local_origin * (BBOX_PADDING + padded_scaling) / padded_scaling;
                            local_direction = local_direction * (BBOX_PADDING + padded_scaling) / padded_scaling; 
                        }
                        ...
                        if (sq_dist <= 1.0f) {
                            float3 local_hit = local_hit_unscaled * scaling_factor;
                                ...
                                float3 local_sphere_hit = local_hit - x * local_direction;
                                const float4* local_to_world = optixGetInstanceInverseTransformFromHandle(optixGetInstanceTraversableFromIAS(params.handle, gaussian_id)); 
                                float3 world_sphere_hit_centered = make_float3(
                                    dot(make_float3(world_to_local[0].x, world_to_local[1].x, world_to_local[2].x), local_sphere_hit),
                                    dot(make_float3(world_to_local[0].y, world_to_local[1].y, world_to_local[2].y), local_sphere_hit),
                                    dot(make_float3(world_to_local[0].z, world_to_local[1].z, world_to_local[2].z), local_sphere_hit)
                                );
                                float3 world_normal = normalize(world_sphere_hit_centered);

                                float3 light_dir = normalize(make_float3(0.5f, -1.0f, 0.5f));
                                float shading = dot(world_normal, light_dir);
                                gaussian_rgb = gaussian_rgb * (max(0.0f, shading) * 0.6f + 0.4f);
                            #else
                    ...
                    #else
                        float alpha = params.all_alphas[TILE_SIZE * TILE_SIZE * idxes[i] + k];
                        float gaussval = params.all_gaussvals[TILE_SIZE * TILE_SIZE * idxes[i] + k];
                        float3 local_hit = params.all_local_hits[TILE_SIZE * TILE_SIZE * idxes[i] + k];
                    #endif
                    ...
                    output_rgb[k] += gaussian_rgb * weight;
                        ...
                        // float3 world_hit = tile_origin + tile_direction * distance;
                    ...
                    #endif
                    #if RENDER_DISTORTION == true
                        float rescaled_distance = distance - DISTORTION_NEAR_PLANE / (DISTORTION_FAR_PLANE - DISTORTION_NEAR_PLANE);

                        output_distortion[k] += (
                            (rescaled_distance * rescaled_distance) * A[k]
                            + D2[k]
                            - 2.0f * rescaled_distance * D[k]
                        );
                    ...
                    cluster_weights[c][k] += weight;
                        ...
                        if (*params.grads_enabled) {
                            if (k == 0) {
                                new_idx = atomicAdd(params.total_hits_for_backprop, 1);
                                params.all_gaussian_ids_for_backprop[new_idx] = gaussian_id; 
                                params.all_prev_hits_for_backprop[new_idx] = params.prev_hit_per_pixel_for_backprop[ray_id]; 
                                params.prev_hit_per_pixel_for_backprop[ray_id] = new_idx;
                            }
                            params.all_distances_for_backprop[TILE_SIZE * TILE_SIZE * new_idx + k] = distance;
                            params.all_local_hits_for_backprop[TILE_SIZE * TILE_SIZE * new_idx + k] = local_hit; 
                            params.all_alphas_for_backprop[TILE_SIZE * TILE_SIZE * new_idx + k] = alpha;
                            params.all_Ts_for_backprop[TILE_SIZE * TILE_SIZE * new_idx + k] = output_t[k].x;
                            params.all_gaussvals_for_backprop[TILE_SIZE * TILE_SIZE * new_idx + k] = gaussval;
                        }
                    #endif
                        ...
                        if (*params.iteration == 0 && ray_id == DEBUG_DUMP_PIXEL_ID && k == 0) {
                            ...
                            params.dump->hit_point_local[dump_i] = params.all_local_hits[idxes[i]];
                            params.dump->gaussval[dump_i] = gaussval;
                            params.dump->alpha[dump_i] = alpha;
                            params.dump->T[dump_i] = next_T;
                            const float4* xform = optixGetInstanceTransformFromHandle(optixGetInstanceTraversableFromIAS(params.handle, gaussian_id));
                    ...
                    if (output_t[0].x < T_THRESHOLD) {
                        goto finished_integration; // todo review a few options instead of this (continue etc)
                    }

                    // #if REMAINING_COLOR_ESTIMATION != RENDER_BUT_DETACH
                    //     printf("transmittance_threshold %f", params.transmittance_threshold);
                    //     if (params.transmittance_threshold > 0.0f) {
                    //         float all_done = true;
                    //         for (int k = 0; k < TILE_SIZE*TILE_SIZE; k++) {
                    //             if (output_t[k].x >= params.transmittance_threshold) {
                    //                 all_done = false;
    ...
    //             float cw = (1.0 - output_t[k].x) / (cluster_weights[c][k] + 1e-12);
    //             #if ATTACH_POSITION == true
    ...
    // #endif
        ...
        register unsigned int gaussian_id_buffer[NUM_STOCH_SAMPLES]; fill_array(gaussian_id_buffer, NUM_STOCH_SAMPLES, NULL_GAUSSIAN_ID);
        register float distances_buffer[NUM_STOCH_SAMPLES]; fill_array(distances_buffer, NUM_STOCH_SAMPLES, 999.9f);
        ...
        for (int i = 0; i < NUM_STOCH_SAMPLES; i++) {
            ...
            #if ATTACH_POSITION == true || POSITION_FROM_EXPECTED_TERMINATION_DEPTH == true
                float3 sample_position = params.gaussian_position[gaussian_id] / NUM_STOCH_SAMPLES;
            #endif
            #if ATTACH_NORMALS == true
                float3 sample_normal = params.gaussian_normal[gaussian_id] / NUM_STOCH_SAMPLES;
            #endif
            #if ATTACH_F0 == true
                float3 sample_f0 = READ_F0(gaussian_id) / NUM_STOCH_SAMPLES;
            #endif
            #if ATTACH_ROUGHNESS == true
                float sample_roughness = READ_ROUGHNESS(gaussian_id) / NUM_STOCH_SAMPLES;
            #endif
            #if ATTACH_SPECULAR == true
                float sample_specular = params.gaussian_specular[gaussian_id] / NUM_STOCH_SAMPLES;
            #endif
            #if ATTACH_ALBEDO == true
                float3 sample_albedo = params.gaussian_albedo[gaussian_id] / NUM_STOCH_SAMPLES;
            #endif
            #if ATTACH_METALNESS == true
                float sample_metalness = params.gaussian_metalness[gaussian_id] / NUM_STOCH_SAMPLES;
            #endif
            #if RENDER_DISTORTION == true
                float sample_distortion = params.gaussian_distortion[gaussian_id] / NUM_STOCH_SAMPLES;
            #endif
        ...
        for (int k = 0; k < TILE_SIZE*TILE_SIZE; k++) {
            ...
            #if ATTACH_POSITION == true
                output_position[k][0] = output_position[k][0] + remaining_T * remaining_position[k][0];
            #endif
            #if POSITION_FROM_EXPECTED_TERMINATION_DEPTH == true
                output_depth[k][0] = output_depth[k][0] + remaining_T * remaining_depth[k][0];
            #endif
            #if ATTACH_NORMALS == true
                output_normal[k][0] = output_normal[k][0] + remaining_T * remaining_normal[k][0];
            #endif
            #if ATTACH_F0 == true
                output_f0[k][0] = output_f0[k][0] + remaining_T * remaining_f0[k][0];
            #endif
            #if ATTACH_ROUGHNESS == true
                output_roughness[k][0] = output_roughness[k][0] + remaining_T * remaining_roughness[k][0];
            #endif
            #if ATTACH_SPECULAR == true
                output_specular[k][0] = output_specular[k][0] + remaining_T * remaining_specular[k][0];
            #endif
            #if ATTACH_ALBEDO == true
                output_albedo[k][0] = output_albedo[k][0] + remaining_T * remaining_albedo[k][0];
            #endif
            #if ATTACH_METALNESS == true
                output_metalness[k][0] = output_metalness[k][0] + remaining_T * remaining_metalness[k][0];
            #endif
            #if RENDER_DISTORTION == true
                output_distortion[k][0] = output_distortion[k][0] + remaining_T * remaining_distortion[k][0];
            #endif
            #if SAVE_LOD_IMAGES == true
                output_lod_mean[k][0] = output_lod_mean[k][0] + remaining_T * remaining_lod_mean[k][0];
                output_ray_lod[k][0] = output_ray_lod[k][0] + remaining_T * remaining_ray_lod[k][0];
            #endif
        }
    #endif 
