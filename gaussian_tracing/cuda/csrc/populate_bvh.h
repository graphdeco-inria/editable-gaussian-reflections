void populateBVH(OptixInstance* instances, OptixTraversableHandle gasHandle, int num_gaussians, float3* camera_position_world, float3* scales, float4* rotations, float3* means, float* opacity, float* lod_means, float* lod_scales, bool* mask, float global_scale_factor, float alpha_threshold, 
