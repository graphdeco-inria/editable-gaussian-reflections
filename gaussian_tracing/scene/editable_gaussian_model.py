...
class EditableGaussianModel(GaussianModel):
    ...
    def get_xyz(self):
        ...
        for key, edit in self.edits.items():
            xyz[self.selections[key].squeeze(1)] += torch.tensor(
    ...
    def _get_rotation(self):  # no activation
        ...
        for key, edit in self.edits.items():
            rotation_angles = torch.deg2rad(
                torch.tensor(
                    [edit.rotate_x, edit.rotate_y, edit.rotate_z],
    ...
    def duplicate_object(self, obj_name: str, offset: float):
        ...
        for key, selection in self.selections.items():
            new_selection = torch.cat(
                (
                    selection,
                    xtra if key in ["Everything", obj_name + "_copy"] else ~xtra,
                ),
                dim=0,
            )
