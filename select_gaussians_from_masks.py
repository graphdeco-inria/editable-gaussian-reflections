...
if __name__ == "__main__":
    ...
    scene = Scene(model_params, gaussians, load_iteration=args.load_iteration, shuffle=False)
    ...
    masks_path = model_params.source_path.replace("/colmap/", "/renders/") + "/test/mask_" + args.mask_name + "/"
    masks = list(tqdm([ 
        TF.to_tensor(Image.open(masks_path + f"/mask_{args.mask_name}_{i:04d}.png").convert("RGB")).cuda() for i in range(len(cams)) 
    ], desc="Loading masks"))
    ...
    for i in range(args.iters):
        ...
        downsized_mask = F.interpolate(mask[None], size=viewpoint.roughness_image.shape[1:], mode="area")[0]
        ...
        if i % 100 == 0:
            print(".")
            save_image(torch.stack([package.roughness[0], downsized_mask]), f"mask.ignore.png")

    torch.save(gaussians._roughness, model_params.model_path + f"/select_{args.mask_name}.pt")
