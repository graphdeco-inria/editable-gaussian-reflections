...
trace_bvh_cuda(int32_t num_rays, int32_t* nodes, float* aabbs,
    ...
    // std::cout << "num_contributes (counting the # of intersections of each ray) time: " << milliseconds << " ms" << std::endl;
    cudaEventRecord(start);

    // *** Now that he know how many intersections there are for each ray, we can allocate the output lists

    // ***  Compute, for each ray, where to start putting values into the output lists. this is computed as a cumsum on the # of intersections for each ray
    ...
    // ***  Since this is a cumulative sum, the last element gives the total # of intersections
    ...
    int32_t num_rendered; // total 
    cudaMemcpy(&num_rendered, ray_offsets + num_rays - 1, sizeof(int32_t), cudaMemcpyDeviceToHost);

    // *** Allocate the output lists. At each index, store: the object id, the hit position, and the ray id
    thrust::device_vector<uint64_t> ray_list_key_vec(num_rendered);
    uint64_t* ray_list_key = thrust::raw_pointer_cast(ray_list_key_vec.data()); // ?? what is ray_list_key_vec
...
//     std::cout << "inclusive_scan time: " << milliseconds << " ms" << std::endl;
    ...
    // *** populate the output lists, which are pre-allocated with the known sizes
    thrust::for_each(thrust::device,
                     ...
                     ] __device__ (uint32_t idx)
                     {
                         ...
                         int32_t* point_ptr=point_list+offset; // object ids
                         float3* position_ptr=position_list+offset; // hit pos
                         int32_t* ray_id_ptr=ray_id_list+offset; // ray id 
    ...
    // ?? perform the z sort here. This is a stable sort, so the order of the rays is preserved. Not certain how this is done exactly
