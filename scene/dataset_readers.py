...
def readCamerasFromTransforms(
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        frames = sorted(contents["frames"], key=lambda frame: frame["file_path"])[
            : model_params.max_images
        ]
...
def readNerfSyntheticInfo(model_params, path, white_background, eval, extension=".png"):
    ...
    train_cam_infos = readCamerasFromTransforms(
        model_params, path, "transforms_train.json", white_background, extension
    )
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(
        model_params, path, "transforms_test.json", white_background, extension
    )
