...
with torch.no_grad(): pc._xyz.nan_to_num_(0.0, 0.0, 0.0)
with torch.no_grad(): pc._scaling.nan_to_num_(0.0, 0.0, 0.0)
with torch.no_grad(): pc._rotation.nan_to_num_(0.0, 0.0, 0.0)
with torch.no_grad(): pc._features_dc.nan_to_num_(0.0, 0.0, 0.0)
...
if False:
    ...
    el = PlyElement.describe(np.array([tuple(v) for v in vertices], dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')]), 'vertex')
...
start = time.time()
hits = raytracer.trace(ray_o.flatten(0, 1), ray_d.flatten(0, 1), pc.get_xyz, pc.get_inverse_covariance(), pc.get_opacity)
...
if False:
    alphas = torch.exp(-(dx).norm(dim=-1)**2*1000.0) * pc.get_opacity[hits.gaussian_ids].squeeze(-1)
else:
...
pixel_colors = nerfacc.accumulate_along_rays(weights=weights, values=colors, ray_indices=hits.ray_ids.long(), n_rays=ray_o.shape[0]*ray_o.shape[1])
...
# --------------------------
save_image(pixel_colors.reshape(1024//2, 1536//2, 3).moveaxis(-1, 0), "render_raycast.png")