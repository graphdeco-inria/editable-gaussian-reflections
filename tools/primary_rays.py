...
def compute_primary_rays(camera_colmap):
    directions = _get_ray_directions(
        1024 // 2, 1536 // 2, [camera_colmap.focal_x / 2, -camera_colmap.focal_y / 2]
    )  # (h, w, 3)
    ray_z = torch.norm(
        directions, dim=-1, keepdim=True
    )  # torch.ones_like(directions[..., :1])  # (h, w, 1)
...
if __name__ == "__main__":
    ...
    class mock_camera:
        R = np.array(
            [
                [8.95075817e-01, -4.21339499e-01, 1.45987016e-01],
                [-4.45913858e-01, -8.45748095e-01, 2.93037404e-01],
                [6.64981544e-09, -3.27388423e-01, -9.44889875e-01],
            ]
        )
        T = np.array([3.60848531e-08, 5.08675688e-08, 4.03112897e00])
