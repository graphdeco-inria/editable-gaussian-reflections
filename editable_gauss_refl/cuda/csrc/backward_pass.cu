...
__device__ void backward_pass(
    ...
    for (auto hit_idx : params.ppll_backward.pixel_view(pixel.id)) {
        ...
        if (step == 0) {
            dL_doutput_rgb = 1.0f / 3.0f * sign(pixel.output_rgb[0] - pixel.target_diffuse) *
                             *params.config.loss_weight_diffuse / num_pixels;
            dL_doutput_depth = 1.0f / 1.0f * sign(pixel.output_depth[0] - pixel.target_depth) *
                               *params.config.loss_weight_depth / num_pixels;
            dL_doutput_normal = 1.0f / 3.0f * sign(pixel.output_normal[0] - pixel.target_normal) *
                                *params.config.loss_weight_normal / num_pixels;
            dL_doutput_f0 =
                1.0f / 3.0f * sign(pixel.output_f0[0] - pixel.target_f0) * *params.config.loss_weight_f0 / num_pixels;
            dL_doutput_roughness = 1.0f / 1.0f * sign(pixel.output_roughness[0] - pixel.target_roughness) *
            ...
            dL_doutput_rgb = 1.0f / 3.0f * sign(output_specular - pixel.target_specular) *
